...
int main(int argc, char* argv[]) {
	...
	int connectivness = 0;	// Percent change that two nodes will be connected
	int random_number;
	...
	for(int i = 0; i < c; i++){
		for(int j = 0; j < r; j++) {
			if(adjacency_matrix[j][i] != MY_INFINITY && adjacency_matrix[i][j] != MY_INFINITY) {
				cout << max(adjacency_matrix[i][j], adjacency_matrix[j][i]) << " \t \t";
			} else if(adjacency_matrix[i][j] != MY_INFINITY) {
