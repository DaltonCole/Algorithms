...
open('data_prim/run_times', 'w').close()
...
for i in range(number_of_graphs):
	...
	# Run Prim's algorithm from the previously randomly generated undirected graph
	proc = subprocess.Popen(["./PRIM/a.out"], stdin=subprocess.PIPE, stdout=subprocess.PIPE, shell=True)
	graph_output = proc.communicate(input=str.encode(graph_input))[0].decode('ascii')
	# End timmer
	run_time = time.time() - start_time
	# Write output of graph to file
	with open('data_prim/results/' + str(i), 'w') as f:
		f.write(graph_output)
	# Append run time to run_time file
	with open('data_prim/run_times', 'a') as f:
...
with open('data_prim/average_run_time', 'w') as f:
