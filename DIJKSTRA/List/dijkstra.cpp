...
void dijkstra() {
	...
	for(int i = 0; i < size; i++) {
		for(int j = 0; j < size; j++) {
			cin >> temp;
			adjacency_list[i].push_back(temp);
		}
	}
	...
	while(pq.empty() != true) {
		...
		if(visited[vertex.number] == false) {
			...
			for(int i = 0; i < adjacency_list[vertex.number].size(); i++) {
				...
				if(adjacency_list[vertex.number][i] != 0 && adjacency_list[vertex.number][i] != INFINITY) {
					// See if updating the distance would improve the distance
					if(distances[i] > distances[vertex.number] + adjacency_list[vertex.number][i]) {
